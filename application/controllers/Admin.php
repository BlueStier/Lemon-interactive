...
class Admin extends CI_Controller
{
    ...
    public function view($nb, $id = false, $data = false)
    {
         ...
         * d'heure.
         * Si c'est le cas redirection à la page de log
         */
    ...
    /* fonction controlant l'enregistrement d'un administrateur */
    public function create_admin()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($exist) {
                $data['error'] = array(
                    'error' => 'Ce pseudo est déjà utiliser veuillez en choisir un autre'
                );
                ...
                if ($mdp != $conf_mdp) {
                    $data['error'] = array(
                        'error' => 'Le mot de passe et sa confirmation sont diffférents'
                    );
                    Admin::view(4, false, $data);
                } else {
    ...
    // fonction de mise à jour d'un admin
    public function update_admin($id)
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($mdp != $conf_mdp) {
                ...
                Admin::view(5, $id, $data);
            } else {
                // mot de passe et confirmation identique on continu en vérifiant si l'admin souhaite conserver sa photo ou en changer
                $conserv = $this->input->post('conserv');
                // oui il souhaite la conserver
                if ($conserv == 'Oui') {
                    $chemin_vers_la_photo = NULL;
                } else {
                    // il souhaite changer de photo on vérifie s'il souhaite l'image par défault
                    $pas_de_photo = $this->input->post('pas_de_photo');
                    if ($pas_de_photo) {
                        // pas de photo choisie
                        $chemin_vers_la_photo = 'assets/admin/image/slider-bg.jpg';
                        ...
                        // on tente l'upload
                        if (! $this->upload->do_upload('photo')) {
                            // si upload hs retour vers la page de update avec info sur l'echec du transfert
                            $data['error'] = array(
                                'error' => $this->upload->display_errors()
                            );
                            Admin::view(5, $id, $data);
                ...
                /*
                 * toutes les infos pour la création d'un admin sont récupérées on appel la fonction
                 * update_admin() du model admin
                 */
                $this->load->model('Admin_model');
                
                $this->Admin_model->update($id, $pseudo, $mdp, $chemin_vers_la_photo, $email);
                // et redirection final
                Admin::view(3);
            }
        }
